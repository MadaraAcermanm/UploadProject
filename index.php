<?php
# -- Developer : Pouria Hosseini | Pv : @NullDeveloper | Channel : @iRoSource -- #
...
$Config = [
    ...
    'admin' => [1084203958, 1084203958, 1084203958], # -- Admin -- #
    'channel' => "NullDeveloper" # -- Channel iD -- #
];
$Database = [
...
ربات دانلود | اشتراک فایل حرفه ای 
به راحتی با چند تا کلیک فایل خودتو دانلود کن❗️",
...
elseif(strpos($text, "/start _") !== false) {
    ...
    if ($dataFile['password']) {
	    ...
	    WSBot('sendmessage', [
        'chat_id' => $chat_id,
        'text' => "💡 ابتدا باید وارد کانال زیر شوید\n\n📣 @{$Config['channel']} 📣 @{$Config['channel']}\n📣 @{$Config['channel']} 📣 @{$Config['channel']}\n\n👇 بعد از « عضویت » بعد از عضویت روی دکمه عضو شدم کلیک کنید 👇", 
        'reply_to_message_id' => $message_id,
        'reply_markup' => json_encode(['inline_keyboard' => [
    [['text' => "📢 عضویت", 'url' => "t.me/".$Config['channel']]],
    [['text' => "👍 عضو شدم", 'callback_data' => "join_$idFile"]],
    ]
])
    ]);
	}	
...
elseif(strpos($data, "join_") !== false) {
    if($crank != "left") {
	$idFile = str_replace("join_", null, $data);
    $dataFile = mysqli_fetch_assoc(mysqli_query($MySQLi, "SELECT * FROM `dbfile` WHERE `code` = '{$idFile}' LIMIT 1"));
        $dl = number_format($dataFile['dl']);
        $method = $dataFile['file'];
        WSBot('send'.$dataFile['file'], [
...
elseif (in_array($from_id, $Config['admin'])) {
	...
	elseif ($user['step'] == 'fortoall') {
		if ($text != "برگشت 🔙") {
		    WSBot('sendmessage', [
			    'chat_id' => $chat_id,
			    'text' => "■ پیام در صف فروارد قرار گرفت .",
			    'reply_to_message_id' => $message_id,
			    'reply_markup' => $panel
			]);
			$db = json_decode(file_get_contents('send.json'), true);
